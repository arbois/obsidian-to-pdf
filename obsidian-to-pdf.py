...
def resolve_wikilinks(md_text, vault_root, temp_dir):
    ...
    def replace_match(m):
        ...
        if page_num:
            ...
            resolved_count += 1
            # Support |Xcm / |Xmm / |Xpt width specification
            if caption and re.match(r'^\d+\.?\d*(cm|mm|pt|em|ex)$', caption):
                return f'![]({image_path}){{width={caption}}}'
...
\setlist[description]{style=nextline, leftmargin=2.5em, font=\normalfont}
\usepackage{etoolbox}
...
def pop_arg(args, flag):
    """Extract --flag value from args list; return (value_or_None, remaining_args)."""
    if flag in args:
        idx = args.index(flag)
        if idx + 1 < len(args):
            value = args[idx + 1]
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3 obsidian-to-pdf.py "filename.md" [--landscape] [--fontsize 10pt] [--parskip 0.4em] [--margin 1.5cm]')
        sys.exit(1)
    ...
    fontsize, args = pop_arg(args, "--fontsize")
    fontsize = fontsize or "12pt"

    parskip, args = pop_arg(args, "--parskip")
    parskip = parskip or "0.7em"

    margin, args = pop_arg(args, "--margin")
    margin = margin or "2cm"
    ...
    try:
        ...
        header = LATEX_HEADER.replace(
            r"\setlength{\parskip}{0.7em}", f"\\setlength{{\\parskip}}{{{parskip}}}"
        )
        ...
        # Use extarticle for non-standard sizes (article only supports 10/11/12pt)
        size_val = fontsize.replace("pt", "").strip()
        try:
            use_extarticle = float(size_val) not in {10.0, 11.0, 12.0}
        except ValueError:
            use_extarticle = False
        ...
        cmd = [
            ...
            "-V", f"geometry:a4paper{',landscape' if landscape else ''}",
            "-V", f"geometry:left={margin},right={margin},top={margin},bottom={margin}",
        ...
        if use_extarticle:
            cmd += ["-V", "documentclass=extarticle"]
